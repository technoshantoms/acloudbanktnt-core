...
BOOST_AUTO_TEST_CASE(custom_auths) { try {
   ...
   BOOST_AUTO_TEST_CASE(authorized_asset_issue_exceptions_1) {
      try {
         ...
         create_user_issued_asset("RSQRCHP1COIN", nathan, white_list);
         create_user_issued_asset("SPECIALCOIN", nathan,  white_list);
         generate_blocks(1);
         const asset_object &nathancoin = *db.get_index_type<asset_index>().indices().get<by_symbol>().find("RSQRCHP1COIN");
   ...
   BOOST_AUTO_TEST_CASE(authorized_override_transfer) {
      try {
         ...
         //////
         upgrade_to_lifetime_member(nathan);
         create_user_issued_asset("RSQRCHP1COIN", nathan, DEFAULT_UIA_ASSET_ISSUER_PERMISSION);
         ...
         //////
         // nathan attempts to override some RSQRCHP1COIN from some account
         // This should succeed because nathan is the issuer
         //////
         ...
         //////
         // nathan authorizes Bob to override transfer RSQRCHP1COIN on its behalf
