...
BOOST_AUTO_TEST_CASE(white_list_asset_rewards_test)
{
   try
   {
      ...
      const asset_id_type nathancoin_id = create_user_issued_asset( "RSQRCHP1COIN", nathan, charge_market_fee|white_list, price, 0, nathancoin_market_percent ).id;
      const asset_id_type nathancoin_id2 = create_user_issued_asset( "RSQRCHP1COIN2", nathan, charge_market_fee|white_list, price, 0, nathancoin_market_percent2 ).id;
      ...
      create_sell_order( elon.id, nathancoin_id(db).amount(1000), nathancoin_id2(db).amount(1500) ); // Elon is willing to sell hes 1000 RSQRCHP1COIN for 1.5 RSQRCHP1COIN2
      create_sell_order(   rob.id, nathancoin_id2(db).amount(1500), nathancoin_id(db).amount(1000) );   // Rob is buying up to 1500 RSQRCHP1COIN2 for up to 0.6 RSQRCHP1COIN
